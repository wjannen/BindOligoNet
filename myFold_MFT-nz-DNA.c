...
void get_stack_energies() {
	...
	*/
	...
	char * wd; 
	char * path = malloc(sizeof(getenv("BINPATH")) + 20);

	if(wd = getenv("BINPATH")) {
		stackstrcat("dnabin/stack.bin",path); 
		...
		stackstrcat("dnabin/tstack.bin",path);
		...
		stackstrcat("dnabin/tstacki.bin",path);
...
void get_iloop_energies(){
	...
	if(wd = getenv("BINPATH")) {
		stackstrcat("dnabin/int1x1.bin",path);
		...
		stackstrcat("dnabin/int2x2.bin",path);
		...
		stackstrcat("dnabin/int2x1.bin",path);
...
int asym_iloop(const int i, const int j, const int n1, const int n2){
	...
	default:
		...
		initiation = -floor((0.89*log((float) sum*1.98 - 4.55) + 2.05) * 100.0);
		break;
	}
